...
async function main() {
    ...
    allRows.forEach(row => {
        ...
        if (!processed[normalizedItemName]) {
            processed[normalizedItemName] = {
                ...
                dailyPrices: {}, // For average calculation
                leagues: {} // Store per-league data
            };
        }
        ...
        // Accumulate per league
        if (!processed[normalizedItemName].leagues[row.League]) {
            processed[normalizedItemName].leagues[row.League] = {};
        }
        if (!processed[normalizedItemName].leagues[row.League][day]) {
             processed[normalizedItemName].leagues[row.League][day] = [];
        }
        processed[normalizedItemName].leagues[row.League][day].push(price);
    });
    ...
    const chartData = Object.keys(processed).map(normalizedName => {
        ...
        const leagueSeries = {};
        Object.keys(itemData.leagues).forEach(leagueName => {
            const leagueDailyData = itemData.leagues[leagueName];
            const lValues = [];
             for (const day in leagueDailyData) {
                const prices = leagueDailyData[day];
                // In case of multiple entries for same day/league (rare but possible), average them
            ...
            lValues.sort((a, b) => a.day - b.day);
            leagueSeries[leagueName] = lValues.filter(v => v.day <= 60);
        });
...
export const availableLeagues = ${JSON.stringify(sortedLeagues)};
